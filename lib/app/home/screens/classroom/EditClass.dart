...
class _EditClassState extends State<EditClass> {
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Flexible(
                  child: TFormField(
                      context, "Final Year", _finalYear, false, false),
                ),
              ],
            ),
            ...
            Row(
              children: [
                ...
                Box(context, "Subjects", buildClassList, _subjects1,
                    SubjectList, Save),
              ],
            ),
            SizedBox(
    ...
    await FirebaseFirestore.instance
      ...
      setState(() {
        ...
        _semester.setDropDown(
          DropDownValueModel(name: value["Semester"], value: value["Semester"]),
        );
              ...
              child: Container(
                child: Card(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        DropField(context, "Select ${_lable}", tList,
                            _controller, true),
                        // SizedBox(height: 10,),
                        ElevatedButton(
                            onPressed: () {
                              setState(() {
                                _teachers1
                                    .add(_controller.dropDownValue!.value);
  ...
  Future buildTeacherList() async {
    await FirebaseFirestore.instance
      ...
      querySnapshot.docs.forEach((doc) {
        setState(() {
          tNames
              .add("${doc["ID"]} - ${doc["First Name"]} ${doc["First Name"]}");
        });
      });
    });
    for (int i = 0; i < tNames.length; i++) {
      setState(() {
        if (!tList.contains(tNames[i])) {
          tList.add(
              DropDownValueModel(name: "${tNames[i]}", value: "${tNames[i]}"));
        }
      });
    }
  }

  Box(context, _lable, Function _buildlist, List _list1,
      List<DropDownValueModel> _list2, Function _function) {
    return Flexible(
      child: InkWell(
        onTap: () async {
          Loading(context);
  ...
  Future buildClassList() async {
    await FirebaseFirestore.instance
        .collection('Courses')
        .doc("Computer")
        .collection(_semester.dropDownValue!.name)
        .get()
        .then((QuerySnapshot querySnapshot) {
      querySnapshot.docs.forEach((doc) {
        setState(() {
